...
app.get('/geteducationdetails', (req, res) => {
    ...
    db.query("SELECT user_details.first_name, user_details.last_name, education_details.* FROM user_details INNER JOIN education_details ON user_details.iduser_details = education_details.userid", [id], (err, result) => {
        if(err)
        {
            console.log(err);
        }
